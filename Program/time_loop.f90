    ...
    do k=1,kUL
        if (Rho(k) .le. 550.) then
            if ((trim(domain) .eq. "FGRN11") .or. (trim(domain) .eq. "FGRN055")) then
                ...
                !MO = 0.8005 + 0.0281*log(acav)      ! fit after debugging heat eq., maximum value
                !MO = 0.5371 - 0.0185*log(acav)      ! fit after debugging heat eq., minimum value
                !MO = 0.6569 + 0.0067*log(acav)      ! Snow density mean annual temp FGRN055, new fit 1957-2020 run
                !MO = 1.
            else
            ...
            if ((trim(domain) .eq. "FGRN11") .or. (trim(domain) .eq. "FGRN055")) then
                ...
                !MO = 1.8306 - 0.1891*log(acav)   ! fit after debuggin heat eq., maximum value
                !MO = 1.6623 - 0.2199*log(acav)   ! fit after debuggin heat eq., minimum value
                !MO = 1.7243 - 0.2011*log(acav)   ! Snow density mean annual temp FGRN055, new fit 1957-2020 run
                !MO = 1.
            else
...
!------------------------------------
    ...
    !if (kUL.gt.2000 .and. Rho(200).ge.rhoi-7. .and. Rho(1).ge.rhoi-7.) call DeleteLayers(kk,kUL,Rho,M,T,Mlwc,DZ, &
    ...
    !if ((Depth(200).gt.100.) .and. (kUL.gt.1500) .and. (sum(Rho(1:200)/200.ge.rhoi-7.) call DeleteLayers(kk,kUL,Rho,M,T,Mlwc,DZ, &
    !    DenRho,Refreeze,Year)
