The fortran model needs to be compiled, and the executable needs to end up in the main FDM folder.
...
The project can be also compiled using the [`Makefile`](./Makefile). We need to set at least two environment variables:

- `NETCDF4_LIB`: `netcdf4-fortran` library path
- `NETCDF4_INCLUDE`: `netcdf4-fortran` include path

as well as specify whether this is for ECMWF (0) or offline (1) compilation:
