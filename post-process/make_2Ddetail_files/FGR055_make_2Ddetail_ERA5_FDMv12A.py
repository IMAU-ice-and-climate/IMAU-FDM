...
os.chdir('/perm/nld3562/code/IMAU-FDM/post-process/make_2Ddetail_files/')

# settings 
variab = "temp"         # dens, temp (can be any output from 2D files)
nyears = 45 		# amount of years 
z_begin = 0		# start at upper most layer
z_end = 50 		# surface snow density up to 13th most upper layers (52 cm) - for 4 cm each layer 
variab_output = "temp"   # SSN: surface snow density or T10m 10-m temp
...
dir3 = "/ec/res4/scratch/nld3562/daily/output/processed_output/"  # directory for 1D output files 
dir4 = "/ec/res4/scratch/nld3562/daily/output/processed_output/"                    # directory save gridded file  
...
# output file name  
fout = dir4 +"FDMv12A_" + variab_output + "layered_1979-2023_ERA5_FGR11.nc"
...
#        TimeY[t] = 1939.02738 + (t/36.5250)             # Fill with fractional years (including leap years)

# create matrix for gridded output 
Matrix_1 = np.empty([n_layer,ntime,n_lat,n_lon])           # Empty 3D array  
...
Matrix_2 = xr.Dataset(data_vars=dict(variab=(["layer","time","rlat","rlon"], Matrix_1.data), lat=(["rlat","rlon"],model_Lat.data), lon=(["rlat","rlon"],model_Lon.data)), coords=dict(time=TimeY)) #, dims=["time","rlat","rlon"])
