...
def make_averages(varname):
    ...
    fname_out = ave_dir / f"{varname}_{config.PROJECT_NAME}_ave.nc"
    if fname_out.exists():
    ...
    try:
        ...
        # Record the spin-up averaging period in the file's metadata so the model
        # can read it from the file instead of parsing it from the filename.
        subprocess.run(
            ["ncatted", "-O",
             "-a", f"start_ave_year,global,o,l,{config.AVE_START_YEAR}",
             "-a", f"end_ave_year,global,o,l,{config.AVE_END_YEAR}",
             str(fname_out)],
            check=True,
        )
    finally:
