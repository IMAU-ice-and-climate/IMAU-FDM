...
--extend-mode append
    ...
    timeseries files. Timeseries files have stable, date-free names
    (e.g. precip_FGRN055_era5_p1.nc), so each is replaced in place with a
    version spanning the original plus the new years. The old version is backed
    up to <name>.nc.bak before being overwritten. The forcing's start/end dates
    live in the file's own metadata (date_bnds/dtg), not the filename.
"""
...
def _stamp_dtobs(path):
    """Stamp the forcing timestep (seconds) into the file as global attr 'dtobs'
    so the model reads it directly instead of parsing the dtg variable."""
    subprocess.run(
        ["ncatted", "-O", "-a", f"dtobs,global,o,l,{config.DTOBS_SECONDS}", str(path)],
        check=True,
    )
...
def make_timeseries(varname, do_parts=True, parts_filter=None):
    ...
    for part in range(1, config.NUM_LONG_BANDS + 1):
        if parts_filter is not None and part not in parts_filter:
            continue
        fname_final = files_dir / f"{varname}_{config.PROJECT_NAME}_p{part}.nc"
...
def extend_timeseries(varname, extend_start, extend_end, mode):
    ...
    # Timeseries files have stable, date-free names; find them by part number.
    existing_files = sorted(files_dir.glob(
        f"{varname}_{config.PROJECT_NAME}_p*.nc"
    ))
    ...
    for existing in existing_files:
        ...
        # Date-free stable name: the output replaces the existing file in place.
        print(f"  Appending {len(new_parts)} new year(s) to part {part} → {existing.name}")
        try:
            _run_ncrcat(["ncrcat", str(existing)] + [str(f) for f in new_parts] + [str(tmp_path)])

            # Back up the old file first (frees the stable name), then write in place.
            bak = existing.with_suffix(".nc.bak")
            existing.rename(bak)
            print(f"  Old file backed up as {bak.name}")

            subprocess.run(
                ["nccopy", "-k", "classic", str(tmp_path), str(existing)],
                check=True,
            )
            _stamp_dtobs(existing)
        finally:
            if tmp_path.exists():
                tmp_path.unlink()
