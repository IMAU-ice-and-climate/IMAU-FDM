"""
Configuration for RACMO pre-processing (FGRN055 / era5).
...
3b. Once satisfied, append new parts to the existing timeseries files:
    ...
    Timeseries files have stable, date-free names (e.g.
    precip_FGRN055_era5_p1.nc), so each is replaced in place with a version
    spanning the original plus new years; the old version is backed up to
    .nc.bak first. The forcing's start/end dates live in the file metadata
    (date_bnds/dtg), not the filename.
...
# Forcing timestep (RACMO output cadence) in seconds; 3-hourly ERA5 = 10800.
# Fixed property of the dataset; stamped into each timeseries file as the global
# attribute 'dtobs' so the model reads it instead of parsing the dtg variable.
DTOBS_SECONDS = 10800
...
# Raw RACMO file suffix (all decade files use the same suffix for ERA5 runs)
FNAME_SUFFIX = f".{DOMAIN}_R23p2.BN_RACMO2.3p2_ERA5_3h_1940_{DOMAIN}.3H.nc"
